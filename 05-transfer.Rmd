...
This chapter introduces _tLDA_, short for transfer-LDA. tLDA enables use cases for fine-tuning from a base model with a single incremental update (i.e., "fine tuning") or with many incremental updates---e.g., on-line learning, possibly in a time-series context---using Latent Dirichlet Allocation. This work uses collapsed Gibbs sampling to implement tLDA [@griffiths2004scientific] but tLDA should be able to be implemented in other MCMC methods [@yao2009streaming] [@lightlda] [@yahoolda] [@chen2015warplda]. tLDA is available for the R language for statistical computing [@rlang] in the _tidylda_ package [@tidylda].

## Related Work

Work on transfer learning with LDA and other probabilistic topic models falls into three categories. The first category contains topic models that explicitly model a topic's evolution over time [@blei2006dtm] [@wang2012dynamic] [@tmlda]. These models differ from true transfer learning in that time is explicitly part of the model, rather than being updated post-hoc. The second category contains models that allow external information to guide the development of topics. External information may be in the form of supervised outcomes [@mcauliffe2007supervised] [@ramage2009labled] [@andrzejewski2009latent], introduced by model structure [@jagarlamudi2012transfer], introduced in the prior [@andrzejewski2009], or constructed interactively with subject matter experts [@hu2014interactive]. The third category contains models designed for incremental and on-line learning [@alsumait2008] [@hoffman2010online] [@rollinglda].

AlSumait, Barbará, and Domeniconi's work, _On-Line LDA: Adaptive Topic Models for Mining Text Streams with Applications to Topic Detection and Tracking_ [@alsumait2008], is especially influential in developing tLDA. Their approach encodes the topics from the previous time slices into the prior, $\boldsymbol\eta$, for the next time slice. More formally

\begin{align}
  \boldsymbol\eta_k^{(t)} = \boldsymbol\omega^{(\delta)} \cdot \boldsymbol\beta_k^{(t-1)}
\end{align}

where $\boldsymbol\eta_k^{(t)}$ is the prior for the $k$-th topic for the current model, $\boldsymbol\omega^{(\delta)}$ is a vector of weights with one entry for the previous $\delta$ models, and $\boldsymbol\beta_k^{(t-1)}$ is a _matrix_ whose rows correspond to the expected value of the posterior distribution of the $k$-th topic for the previous $\delta$ models. In essence, $\boldsymbol\eta_k^{(t)}$ is a weighted linear combination of the last $\delta$ posterior distributions for the $k$-th topic. The weights sum to $1$ such that $\sum_{j=t-\delta-1}^{t-1} \omega_j = 1$. There are two mechanical differences between On-Line LDA and tLDA. First, when $\delta > 1$,  On-Line LDA loses the Markov property and can only be applied to time series use cases. Second, if $\delta = 1$, then $\omega = 1$. tLDA does not have a restriction on the size of the weight.
...
\begin{align}
\omega_k^{(t)} &=
a^{(t)} \cdot \sum_{v = 1}^V Cv_{k,v}^{(t-1)} + \eta_{k,v}^{(t-1)} \label{eq:priorweight}
\end{align}

Appendix D shows the full derivation of the relationship between $a^{(t)}$ and $\omega_k^{(t)}$.
...
The _tidylda_ package---described in Chapter 6---implements an algorithm for tLDA in 6 steps.
...
Step 1 uses the relationship in (\ref{eq:priorweight}), above. Step 2 uses a standard prediction method for LDA models. The _tidylda_ implementation uses a dot-product prediction described in Appendix A for speed. MCMC prediction would work as well.
...
where $\hat{\boldsymbol\beta}_{k,n}^{(t)}$ is the estimated probability of choosing the $n$-th word from topic $k$ in the $t$-th period and $\hat{\boldsymbol\theta}_{d,k}^{(t)}$ is the estimated probability of choosing the $k$-th topic from the $d$-th document in the $t$-th period.
...
1. $\left(\sum_{v=1}^V \eta_v \right) \in \{50, 250, 500, 1000\}$
...
[^weirdburnin2]: It is common to burn in half of the total number of iterations. I chose to burn in over the last quarter for computational reasons. Specifically, I wanted at least 150 burn in iterations but due to computation time, limited the total number of iterations to 200.
...
I evaluate the models using pairwise Hellinger distance [@hellinger1909] calculated from the estimated topics to each of the ground truth simulated topics. For estimated topic $\hat{\boldsymbol\beta}_k^{(t)}$ and ground-truth topic $\boldsymbol\beta_{\mathfrak{k}}$, the Hellinger distance is calculated as
...
\begin{align}
    \underset{t \to \infty}{\lim} 
      P(H_{k,\mathfrak{k}}^{'(t)} \geq \epsilon) = 0 \label{eq:cprob} 
\end{align}

then topic $\boldsymbol\beta^{(t)}_k$ converges in probability to $\boldsymbol\beta_\mathfrak{k}$. (\ref{eq:cprob}) is equivalent to 

\begin{align}
        \underset{t \to \infty}{\lim} 
      H_{k,\mathfrak{k}}^{(t)} = \epsilon  
\end{align}

And since $\epsilon$ is a constant, that would imply
...
I introduced additional models for $a = 0.65$ to $a = 0.95$---with a step size of $0.05$---to further examine the region around $a = 0.8$. The transition between divergence to convergence happens quickly as shown at the top of Figure \@ref(fig:tlda-mean-var). And the bottom of Figure \@ref(fig:tlda-mean-var) reveals that $a = 0.8$ is the point of maximum variance in p-values of tests measuring whether or not a model converges.


```{r tlda-mean-var, fig.width = 5, fig.height = 3, fig.cap = "P-values assessing convergence for each value of a with the mean in read (top) and the variance in blue (bottom). Individual observations are black points (top)."}

bin_table_sum <- 
  bin_table |>
  group_by(a) |>
  summarize(
    mean_p = mean(bin_test_p),
    var_p = var(bin_test_p)
  )

p1 <- bin_table_sum |>
  ggplot() +
  geom_line(aes(x = a, y = mean_p), lwd = 1.5, col = "red") + 
  geom_point(data = bin_table, aes(x = a, y = bin_test_p)) + 
  xlab("") +
  ylab("P-value") + 
  geom_vline(aes(xintercept = 0.8), lty = 2) + 
  geom_vline(aes(xintercept = 0.7), lty = 3) + 
  geom_vline(aes(xintercept = 0.9), lty = 3) +
  scale_x_continuous(breaks = seq(0.2, 2, by = 0.1)) + 
  theme(
...
p2 <- bin_table_sum |>
  ggplot() +
  geom_line(aes(x = a, y = var_p), lwd = 1.5, col = "blue") + 
  xlab("a") +
  ylab("Var(p-value)") + 
  geom_vline(aes(xintercept = 0.8), lty = 2) + 
  geom_vline(aes(xintercept = 0.7), lty = 3) + 
...
One other area I leave for future study is using tLDA to build large pre-trained language models, as is common with deep learning transformers like BERT [@bert2018] and the GPT [@gpt2] [@gpt3] family. This is similarly possible in principle. However, the Gibbs algorithm used here paper makes building such large models computationally infeasible. Algorithms exist for scaling LDA estimates to "web-scale" data,  such as [@chen2015warplda], which can be used to implement tLDA in the future.
