...
Figure \@ref(fig:sbir-vary-a-1) plots these four statistics for each value of $a$ over time. A clear pattern appears for mean Hellinger distance but not for other statistics. Mean Hellinger distance is highest when $a$ is smallest and decrease as $a$ grows. This is consistent with expectation. When $a$ is small, the prior has less weight in the posterior and the data has more. This will lead to a posterior that changes significantly period to period as it is more-strongly shaped by the data. 
...
\sum_{d = 1}^D n_d \cdot \theta_{d,k} \cdot \boldsymbol\beta_k (\#eq:prevalence)
\end{align}
